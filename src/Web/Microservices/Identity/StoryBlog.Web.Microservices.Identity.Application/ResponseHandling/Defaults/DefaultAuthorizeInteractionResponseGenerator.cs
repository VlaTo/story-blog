...
public class DefaultAuthorizeInteractionResponseGenerator : IAuthorizeInteractionResponseGenerator
{
    /// <summary>
    /// The logger.
    /// </summary>
    ...
    /// <summary>
    /// Initializes a new instance of the <see cref="DefaultAuthorizeInteractionResponseGenerator"/> class.
    /// </summary>
    ...
    public DefaultAuthorizeInteractionResponseGenerator(
        IdentityServerOptions options,
        ISystemClock clock,
        ILogger<DefaultAuthorizeInteractionResponseGenerator> logger,
    ...
    public virtual async Task<InteractionResponse> ProcessInteractionAsync(ValidatedAuthorizeRequest request, ConsentResponse? consent = null)
    {
        ...
        if (consent is { Granted: false, Error: { } })
        {
        ...
        if (result is { IsLogin: false, IsError: false, IsRedirect: false })
        {
    ...
    protected internal virtual async Task<InteractionResponse> ProcessLoginAsync(ValidatedAuthorizeRequest request)
    {
        ...
        if (isAuthenticated && request is { Subject: not null, Client: not null })
        {
            var context = new IsActiveContext(request.Subject, request.Client, IdentityServerConstants.ProfileIsActiveCallers.AuthorizeEndpoint);
        ...
        if (false == isAuthenticated || false == isActive)
        {
        ...
        {
            if (false == request.Client.EnableLocalLogin)
            {
        ...
        else if (null != request.Client?.IdentityProviderRestrictions &&
    ...
    protected internal virtual async Task<InteractionResponse> ProcessConsentAsync(ValidatedAuthorizeRequest request, ConsentResponse consent = null)
    {
        ...
        if (request.PromptModes.Any() &&
            false == request.PromptModes.Contains(OidcConstants.PromptModes.None) &&
            false == request.PromptModes.Contains(OidcConstants.PromptModes.Consent))
        {
            ...
            else
            {
                ...
                else
                {
                    ...
                    else
                    {
                        ...
                        {
                            // remember consent
                            IList<ParsedScopeValue> parsedScopes = Array.Empty<ParsedScopeValue>();
