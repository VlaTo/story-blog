...
public class IntrospectionResponseGenerator : IIntrospectionResponseGenerator
{
    ...
    public virtual async Task<Dictionary<string, object>> ProcessAsync(IntrospectionRequestValidationResult validationResult)
    {
        ...
        // token is invalid
        if (false == validationResult.IsActive)
        {
        ...
        if (false == await AreExpectedScopesPresentAsync(validationResult))
        {
            return response;
        }
        ...
        response = validationResult.Claims
        ...
        var scopes = validationResult.Claims
            .Where(c => c.Type == JwtClaimTypes.Scope)
            .Select(x => x.Value);
