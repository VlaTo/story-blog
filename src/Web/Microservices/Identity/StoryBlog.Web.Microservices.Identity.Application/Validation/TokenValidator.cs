...
internal sealed class TokenValidator : ITokenValidator
{
    ...
    public async Task<Results.TokenValidationResult> ValidateIdentityTokenAsync(string token, string? clientId = null, bool validateLifetime = true)
    {
    ...
    public async Task<Results.TokenValidationResult> ValidateAccessTokenAsync(string token, string? expectedScope = null)
    {
        ...
        Results.TokenValidationResult result;
        ...
        if (token.Contains("."))
        {
            if (token.Length > options.InputLengthRestrictions.Jwt)
            {
                ...
                return new Results.TokenValidationResult
                {
                    IsError = true,
    ...
    private async Task<Results.TokenValidationResult> ValidateJwtAsync(
        string jwtString,
    ...
    private async Task<Results.TokenValidationResult> ValidateReferenceAccessTokenAsync(string tokenHandle)
    {
    ...
    private static Results.TokenValidationResult Invalid(string error)
    {
        return new Results.TokenValidationResult
        {
            IsError = true,
