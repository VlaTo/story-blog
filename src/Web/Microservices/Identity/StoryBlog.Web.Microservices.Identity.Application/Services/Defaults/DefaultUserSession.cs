...
internal class DefaultUserSession : IUserSession
{
    ...
    public async Task<string?> CreateSessionIdAsync(ClaimsPrincipal principal, AuthenticationProperties properties)
    {
        ...
        if (null == properties.GetSessionId())
        {
            ...
            {
                var uniqueId = CryptoRandom.CreateUniqueId(16, CryptoRandom.OutputFormat.Hex);
                properties.SetSessionId(uniqueId);
            }
        }
    ...
    public async Task EnsureSessionIdCookieAsync()
    {
        ...
        if (null != sessionId)
        {
            IssueSessionIdCookie(sessionId);
        }
        else
        {
    ...
    {
        if (null != HttpContext && null != CheckSessionCookieName)
        {
    ...
    public async Task AddClientIdAsync(string clientId)
    {
        ...
        if (null != Properties)
        {
            ...
            if (false == clientIds.Contains(clientId))
            {
    ...
    public virtual void IssueSessionIdCookie(string? sessionId)
    {
        if (Options.Endpoints.EnableCheckSessionEndpoint && null != CheckSessionCookieName)
        {
            var actualId = HttpContext?.Request.Cookies[CheckSessionCookieName];

            if (false == String.Equals(actualId, sessionId))
            {
                var sessionCookieName = Options.Authentication.CheckSessionCookieName;

                if (null != sessionCookieName && null != sessionId)
                {
    ...
    {
        if (null == Principal || null == Properties)
        {
            if (null != HttpContext)
            {
