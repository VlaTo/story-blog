...
internal class AuthenticationRequestHandlerWrapper : IAuthenticationRequestHandler
{
    ...
    public async Task<bool> HandleRequestAsync()
    {
        ...
        if (result)
        {
            ...
            if (null != context && context.GetSignOutCalled() && StatusCodes.Status200OK == context.Response.StatusCode)
            {
    ...
    {
        logger?.LogDebug("Processing federated signout");

        var iframeUrl = await context.GetIdentityServerSignOutFrameCallbackUrlAsync();

        if (null != iframeUrl)
        {
            logger?.LogDebug("Rendering signout callback iframe");
            await RenderResponseAsync(context, iframeUrl);
        }
        else
        {
            logger?.LogDebug("No signout callback iframe to render");
        }
    }
